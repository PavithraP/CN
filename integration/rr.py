...
def drawPtr(noOfInterface,val,up,gr):
	tlist = list()
	screen = turtle.getscreen()	
	screen.tracer(0)
	for i in range(noOfInterface):
	    	tlist.append(turtle.Turtle())
		setPos(tlist[i],val-10*i,up)
		if gr == i:
			tlist[i].begin_fill()
			tlist[i].circle(5)
			tlist[i].end_fill()
		...
		for i in range(noOfInterface):
			for j in range(noOfInterface):
				...
				if(sending[i][j] == 1):	#acceptance = [[0 for j in range(n)] for i in range(n)]
					...
					count += 1
					sending[i][j]=0
					print "voq*********",voqList[i] , j ,acceptance[i]
					#if j in voqList[i]:
					voqList[i].remove(j)
	...
	for i in range(noOfInterface):
		if(len(request[i]) > 0):
			while grantPtr[i] not in request[i]:
				grantPtr[i] = (grantPtr[i] + 1) % noOfInterface 
			grant[grantPtr[i]].append(i)
			grantPtr[i] = (grantPtr[i] + 1) % noOfInterface
			print "grant ",i,grantPtr[i]
		drawPtr(noOfInterface,-150,150-(10+90*i),grantPtr[i])
	return grant
	...
	for i in range(noOfInterface):
		if(len(grant[i]) > 0):
			while acceptPtr[i] not in grant[i]:
				acceptPtr[i] = (acceptPtr[i] + 1) % noOfInterface
		...
		drawPtr(noOfInterface,-300,150-(10+90*i),acceptPtr[i]+1)
